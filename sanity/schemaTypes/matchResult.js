export default {
  ...
  fields: [
    ...
    {
      name: 'amateurGrade',
      title: 'Amateur Grade (Only for Amateurs)',
      type: 'string',
      options: {
        list: [
      ...
      hidden: ({document}) => document?.competition !== 'Amateur'
    },
    {
      name: 'countryLeague',
  ...
  preview: {
    ...
    prepare({homeTeam, awayTeam, homeScore, awayScore, competition, amateurGrade}) {
      return {
        title: `${homeTeam} ${homeScore} - ${awayScore} ${awayTeam}`,
        subtitle: amateurGrade ? `${competition} · ${amateurGrade}` : competition
      }
    }
  }
}