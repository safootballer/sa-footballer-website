...
export default function LadderContent() {
  ...
  const tabBtn = (active, color = '#2ca3ee') => ({
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-5xl mx-auto">
            <div className="bg-[#2ca3ee] text-white px-6 py-4 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold">{level1}</h2>
                <p className="text-sm opacity-80">{getLeagueCategory(getGradeId(activeDoc)).level3} · {activeDoc.season}</p>
              </div>
              {activeDoc.syncedAt && (
