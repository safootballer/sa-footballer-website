      ...
      {videoId ? (
        <div className="relative pb-[56.25%] bg-black">
          {playing ? (
            ...
            <button onClick={() => setPlaying(true)} className="absolute top-0 left-0 w-full h-full group" aria-label={`Play ${title}`}>
        ...
        <div className="w-full h-48 bg-gray-200 flex items-center justify-center"><span className="text-gray-500">Video unavailable</span></div>
      )}
      <div className="p-4">
        <h3 className="font-bold mb-2 line-clamp-2 text-gray-800">{title}</h3>
        {category && (
          <p className="text-sm font-semibold text-red-600 mb-2">{category === 'live-stream' ? 'Live Stream' : 'Filming'}</p>
        )}
        {date && (
          <p className="text-sm text-gray-700">{new Date(date).toLocaleDateString('en-AU', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        )}
      </div>
    </div>
  )
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="border-t border-gray-700 pt-8">
              <a href="/contact" className="inline-block bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-full font-bold transition text-sm">
                Get a Quote
              </a>
            </div>
          </div>
              ...
              <>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {dedupedYoutube.map((video) => (
                    <VideoCard key={video.videoId} videoId={video.videoId} title={video.title} date={video.publishedAt} />
                  ))}
                </div>
              </>
            )}
